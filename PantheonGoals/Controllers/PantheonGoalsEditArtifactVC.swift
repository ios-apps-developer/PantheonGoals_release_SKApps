...
protocol PantheonGoalsEditArtifactDelegate: AnyObject {
    func artifactDidUpdate()
}

final class PantheonGoalsEditArtifactVC: UIViewController {
    ...
    weak var delegate: PantheonGoalsEditArtifactDelegate?
    ...
    private func loadData() {
        nameTextField.text = artifact.name
        targetAmountTextField.text = String(format: "%.0f", artifact.targetAmount)
        selectedImage = artifact.imageName
    ...
    @objc private func saveButtonTapped() {
        ...
        if let dateText = deadlineLabel.text,
           ...
           let date = dateFormatter.date(from: dateText.replacingOccurrences(of: "Deadline: ", with: "")) {
            deadline = date
        }
        
        // Update artifact
        artifact.name = name
        artifact.targetAmount = amount
        artifact.imageName = imageName
    ...
    private func showDatePicker() {
        ...
        alert.addAction(UIAlertAction(title: "Done", style: .default) { [weak self] _ in
            let formatter = DateFormatter()
            formatter.dateStyle = .short
            self?.deadlineLabel.text = "Deadline: \(formatter.string(from: datePicker.date))"
...
extension PantheonGoalsEditArtifactVC: UICollectionViewDataSource {
...
extension PantheonGoalsEditArtifactVC: UICollectionViewDelegateFlowLayout {
